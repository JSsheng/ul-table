...
import { hasCompleteOverlap, intersects, keys, replaceAtPosition, htmlEntities, indexOfGroup } from "./utils";
...
class Prism {
  ...
  constructor(options: Options) {
       ...
       * @type {Object}
       */
      const replacements = {} as Record<string, { with: string; replace: string }>;
       ...
       * @type {Object}
       */
      const replacementPositions = {} as Record<string, number>;
      ...
      function _matchIsInsideOtherMatch(start: number, end: number) {
          for (let key in replacementPositions) {
              ...
              if (hasCompleteOverlap(k, replacementPositions[k], start, end)) {
                  delete replacementPositions[k];
                  delete replacements[k];
              }

              if (intersects(k, replacementPositions[k], start, end)) {
                  return true;
              }
          }
       ...
       * @return {string}
       */
      function _wrapCodeInSpan(name: string, code: string) {
       ...
       * @return {string}
       */
      function _processReplacements(code: string) {
          const positions = keys(replacements);
          for (const position of positions) {
              const p = parseInt(position, 10);
              const replacement = replacements[position];
              code = replaceAtPosition(p, replacement.replace, replacement.with, code);
          }
          return code;
      }
       ...
       * @return {string}
       */
      function _cloneRegex(regex: RegExp) {
          let flags = '';
       ...
       * @return {mixed}
       */
      function _processPattern(pattern: any, code: string, offset = 0) {
          ...
          const match = regex.exec(code) as RegExpExecArray;
          if (!match) {
              return false;
          }
          ...
          function _processGroup(groupKey: number) {
               ...
               */
              const _getReplacement = function(passedBlock: string, replaceBlock: string, matchName?: string) {
          ...
          for (const groupKey of groupKeys) {
              _processGroup(parseInt(groupKey));
          }
       ...
       * @return {string}
       */
      function _processCodeWithPatterns(code: string, patterns: Pattern[]) {
       ...
       * @return {Array}
       */
      function getPatternsForLanguage(language: string): Pattern[] {
          let patterns = options.patterns?.[language] || [];
          while (options.inheritenceMap?.[language]) {
              language = options.inheritenceMap[language];
              patterns = patterns.concat(options.patterns?.[language] || []) as Pattern[];
          }
          return patterns;
      }
       ...
       * @return {string}
       */
      function _highlightBlockForLanguage(code: string, language: string, patterns: Pattern[]) {
